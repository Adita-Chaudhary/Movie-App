...
describe('Home page (integration, StrictMode)', () => {
  ...
  it('runs the full personalized pipeline (profile -> shortlist -> enrichment -> re-rank -> explanations) and excludes already-rated movies even when poorly rated', async () => {
    ...
    // The explanation must NOT be permanently visible under every card
    // (that's the exact clutter this UI was changed to avoid) - only
    // reachable through the compact "Why" info badge.
    expect(getRecommendationSection().textContent).not.toContain('Recommended because');

    const whyBadge = getRecommendationSection().querySelector('button[aria-label^="Why "]');
    expect(whyBadge).toBeTruthy();

    // Hovering the badge reveals the same honest, matched-signal
    // explanation (genre overlap with "Loved Movie"'s Action genre) -
    // still generated by the real explainRecommendation(), just
    // surfaced on demand instead of inline.
    const user = userEvent.setup();
    await user.hover(whyBadge);

    await waitFor(() => {
      const tooltip = document.querySelector('[role="tooltip"]');
      expect(tooltip).toBeTruthy();
      expect(tooltip.textContent).toContain('Action');
    });
  });
});
